...
public class Drivetrain extends Subsystem {
	...
	private HashMap<RobotMap, WPI_TalonSRX> motorMap = new HashMap<RobotMap, WPI_TalonSRX>();
	...
	public Drivetrain() {
		...
		motorMap.values().forEach((motor) -> {
			motor.setNeutralMode(NeutralMode.Brake);
		});

		differentialDrive = new DifferentialDrive(speedControllerGroupLeft, speedControllerGroupRight);

		gearBox = new DoubleSolenoid(RobotMap.COMPRESSOR.getValue(), RobotMap.GEARBOX_1.getValue(),
	...
	public void arcadeDrive(double speed, double turn, double multiplier) {
		differentialDrive.arcadeDrive(Math.copySign(speed * speed, speed) * multiplier,
				Math.copySign(turn * turn, turn) * multiplier, false);
	}
	...
	public HashMap<RobotMap, WPI_TalonSRX> getMotorMap() {
		return motorMap;
	}

	public SpeedControllerGroup getSpeedControllerGroupLeft() {
		return speedControllerGroupLeft;
	}

	public SpeedControllerGroup getSpeedControllerGroupRight() {
		return speedControllerGroupRight;
	}
	...
	public DoubleSolenoid getGearBox() {
		return gearBox;
	}
}
